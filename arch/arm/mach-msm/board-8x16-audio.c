...
static int card_on = 0;
static int modem_state = 0;
...
static struct acoustic_ops acoustic = {
	...
	.get_snd_card = msm8x16_get_snd_card
};
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("%s: mi2s_rx_bit_format = %d, ucontrol value = %ld\n",
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	pr_info("%s: msm_pri_mi2s_rx_ch = %d\n", __func__, msm_pri_mi2s_rx_ch);
	return 1;
}
...
static int conf_int_codec_mux(struct msm8916_asoc_mach_data *pdata)
{
	...
	val = ioread32(vaddr);
	val = val | 0x00220002;
	pr_info("%s: PRI mux configuration = %x\n", __func__, val);
	iowrite32(val, vaddr);
	iounmap(vaddr);
	return ret;
}
...
static int msm_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if (!pdata->codec_type) {
		...
		if (ret < 0) {
			pr_err("%s failed to enable the pri mi2s sclk %x\n",
					__func__,
		...
		if (ret < 0) {
			pr_err("ext_mi2s_clk_ctl failed, ret: %d\n", ret);
			return ret;
		}
	}
...
static int msm8x16_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (apr_get_modem_state() != APR_SUBSYS_LOADED) {
		pr_err("%s: modem is not ready yet, probe defer return\n", __func__);
		return -EPROBE_DEFER;
	} else if (modem_state == 0) {
		pr_err("%s: modem state is 0 , probe defer return\n", __func__);
		modem_state = 1;
		return -EPROBE_DEFER;
	}
	...
	if (!pdata) {
		pr_err("Can't allocate msm8x16_asoc_mach_data\n");
		ret = -ENOMEM;
		goto err;
	}
